...
void cfgCom::iniCombos()    //  carrega opções dos combos, exceto Port
{
...
bool cfgCom::atuPorts()     //  carrega opções de portas e retorna true ou false, de acordo com a existência de portas
{
    ui->cbPort->clear();
...
bool cfgCom::cfgPorta()
{
    ...
    return cSerial.open(QIODevice::ReadOnly);
}
...
void cfgCom::on_pbOk_clicked()
{
    if(!flg)
    {
        if(cfgPorta())
        {
            ...
            ui->pbOk->setText("OK");
            ui->frame->setStyleSheet("background-color: lime; border: 2px solid darked; border-radius: 10px;");
        }
        return;
    }
    this->close();
